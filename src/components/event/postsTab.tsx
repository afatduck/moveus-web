    ...
    return <div className="flex flex-col overflow-auto grow">
        {
            posts.map(post => <div key={post.id} className="rounded-[15px] box-border p-6 bg-block relative">
                ...
                <div className="usertab mt-auto mb-8">
                    <UserImage canChange={false} userId={organizer.user.id!} className="inline-block"/>
                    <div className="inner">
                        <div className="tags">
                            <p>organizer</p>
                        </div>
                        <h4>{displayName(organizer.user.username, organizer.user.firstName, organizer.user.lastName )}</h4>
