...
import { EventFragment, useCancelEventMutation, useJoinEventMutation, useLeaveEventMutation } from "~/graphql/generated"
    ...
    return <div>
        <div className="flex gap-3 mb-4">
            ...
            </button>
            {
                ...
                <button className="bg-block-accent" onClick={handleCancel}>Cancel</button> :
        ...
        </div>
        {
            ...
            (
                event.role === null ?
                (joinLoading ? <HashLoader className="mx-auto" color={LOADER_COLOR} /> : <button onClick={handleJoin}>Join</button>):
                (leaveLoading ? <HashLoader className="mx-auto" color={LOADER_COLOR} /> : <button className="bg-block text-foreground" onClick={handleLeave}>Leave</button>)
            )
        }
    </div>
}
