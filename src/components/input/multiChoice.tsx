...
function MultiChoice<T,> ({value, setValue, options, classname }: MultiChoiceProps<T>) {
    ...
    return <div className={"flex flex-col w-full gap-3 " + classname}>
        {
            options.map(option => 
            <button key={option.name} onClick={() => handleClick(option.value)}
                className={"text-left px-6 " + 
                    (value.includes(option.value) ? 
                    "" :
                    "text-foreground bg-block")
                }>
                {option.name}
            </button>)
        }
