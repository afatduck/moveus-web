...
function UserImage({userId, canChange, className}: UserImageProps) {
    ...
    const [image, setImage] = useState<string>(`${process.env.REACT_APP_BUCKET_URL}profile-pictures/${userId}`)
    ...
    const onSelectFile = useCallback((e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.addEventListener('load', () => setInputImage(reader.result as string));
    ...
    const handleUpload = useCallback((cropped: string) => {
        apolloClient.query({
    ...
    {inputImage && <InputImage image={inputImage!} handleCancel={handleCancel} handleUpload={handleUpload} />}
    <input
        type="file"
        accept="image/*"
        ref={inputRef}
