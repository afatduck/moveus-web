...
import { GetAttachmentUploadUrlDocument, useSendMessageMutation, WsChatMessageType } from "~/graphql/generated"
import { fileToBase64 } from "~/utils/imageData"
...
function SendMessage ({ chatId, addMessage }: SendMessageInterface) {
    ...
    const handleSubmit = useCallback(async (e: FormEvent) => {
        ...
        if (selectedImage) {
            await apolloClient.query({
                ...
                const blob = new Blob([new Uint8Array(buffer)], {type: selectedImage.type})
                await fetch(url, {
                    method: "PUT",
                    body: blob,
                    headers: {
    ...
    }, [text, selectedImage, clearImageInput, setText, chatId, sendMessage, addMessage])

    return <div className="my-8 flex flex-col gap-4">
    {selectedImage && <button className="!text-sm !px-2 !py-1 !inline !w-auto bg-block-accent"
            onClick={clearImageInput}>
            Image selected, click to remove
        </button>}
    <form className="px-4 py-2 border border-foreground rounded-[15px] flex items-center gap-3 w-full" onSubmit={handleSubmit}>
