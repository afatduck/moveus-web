...
import EventInfoTab from "~/components/event/infoTab";
import EventParticipantsTab from "~/components/event/participantsTab";
import EventPostsTab from "~/components/event/postsTab";
        ...
        <BackButton />
        {
            (() => {
                switch (true)
                {
                    ...
                    default:
                        return <div className="grow flex flex-col pb-8 max-h-[calc(100%-2rem)]">
                            <div className="flex justify-around py-2 border-b border-foreground bottom-1 [&>p]:cursor-pointer [&>p]:select-none my-4">
                                <p className={tab === "info" ? "text-block-accent" : ""} onClick={() => setTab("info")}>info</p>
                                <p className={tab === "participants" ? "text-block-accent" : ""} onClick={() => setTab("participants")}>participants({event!.members.length})</p>
                                <p className={tab === "posts" ? "text-block-accent" : ""} onClick={() => setTab("posts")}>posts</p>
                            </div>
                            <div className="h-4 shrink-0" />
                            {
                                (() => {
                                    switch (tab) {
                                        case ("info"): return <EventInfoTab event={event!} />
                                        case ("participants"): return <EventParticipantsTab members={event!.members} />
                                        case ("posts"): return <EventPostsTab posts={event!.posts} organizer={event!.organizer!} />
                                    }
                                })()
                            }
